...
class Trainer(object):
    ...
    def train(self, start_epoch, end_epoch, start_time=None):
        ...
        print('best recall@20: ', best_recall, 'best mrr: ', best_mrr)
        res_df = pd.DataFrame({
            "exp_loss_type": [self.args.loss_type],
            "recall@20": [best_recall],
            "mrr": [best_mrr]
        })
        res_df.to_csv(os.path.join(self.args.result_dir, self.args.loss_type+".csv"), index = False)
